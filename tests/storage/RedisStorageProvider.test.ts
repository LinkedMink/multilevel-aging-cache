...
import { getStringKeyJsonValueOptions, IRedisStorageProviderOptions } from "../../src/storage/IRedisStorageProviderOptions";
...
describe("RedisStorageProvider.ts", () => {
  ...
  beforeEach(() => {
    ...
    provider = new RedisStorageProvider(clientMock, optionsMock, channelMock);
  })

  test("should return instance when constructor parameters are valid", () => {
    const provider = new RedisStorageProvider(new Redis(), getStringKeyJsonValueOptions(), new Redis());
  ...
  test("should call Redis client get and deserialized stored value when get is called", () => {
    ...
    const testSerializedValue = { value: mockSerializer.serialize(testValue), age: 0 };
    const getSpy = jest.spyOn(clientMock, "get").mockReturnValue(
      Promise.resolve(JSON.stringify(testSerializedValue)));
  ...
  test("should call Redis client set with serialized key/value when set is called", () => {
    const setSpy = jest.spyOn(clientMock, "set").mockReturnValue(Promise.resolve("OK") as unknown as undefined);
    ...
    return promise.then(isSuccessful => {
      ...
      expect(publishSpy).toHaveBeenCalledWith(
        ...
        JSON.stringify({ key: mockSerializer.serialize(testKey), age: testValue.age, value: mockSerializer.serialize(testValue.value) }));
    });
  });
  ...
  test("should call Redis client keys when keys is called", () => {
    ...
    const keysSpy = jest.spyOn(clientMock, "keys").mockReturnValue(
  ...
  test("should notify subscribers of delete when message is published", () => {
    const testKey = "TEST_KEY";
    const testMessage = JSON.stringify({ key: mockSerializer.serialize(testKey) });
    const subscriber = jest.fn();
    provider.subscribe(subscriber);
