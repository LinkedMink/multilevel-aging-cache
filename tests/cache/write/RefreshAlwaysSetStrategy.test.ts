...
describe("RefreshAlwaysSetStrategy.ts", () => {
  ...
  test("should execute set unconditionally when set() is called with force", () => {
    ...
    hierarchyMock.getValueAtTopLevel = jest.fn().mockResolvedValue({ age: testAge + 20, value: testValue });
    ...
    return promise.then(status => {
      ...
      expect(hierarchyMock.setAtLevel).toBeCalledWith(testKey, { value: testValue, age: testAge });
    })
  });
  ...
  test("should not execute set when set() is called and highest level has key", () => {
    ...
    return promise.then(status => {
      ...
      expect(hierarchyMock.setAtLevel).not.toBeCalledWith(testKey, { value: testValue, age: testAge });
