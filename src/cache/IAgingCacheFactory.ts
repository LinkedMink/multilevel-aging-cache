...
import { getDefaultAgingCacheOptions, AgingCacheWriteMode, IAgingCacheOptions, AgingCacheReplacementPolicy } from "./IAgingCacheOptions";
...
import { IAgingCacheSetStrategy, IAgingCacheDeleteStrategy } from "./write/IAgingCacheWriteStrategy";
...
type IAgingCacheSetStrategyConstructor = new <TKey, TValue>(
  ...
  evictQueue: IAgedQueue<TKey>) => IAgingCacheSetStrategy<TKey, TValue>;
type IAgingCacheDeleteStrategyConstructor = new <TKey, TValue>(
  hierarchy: IStorageHierarchy<TKey, TValue>,
  evictQueue: IAgedQueue<TKey>) => IAgingCacheDeleteStrategy<TKey>;
...
export function checkAgingCacheOptionsValid(options: IAgingCacheOptions): Error | undefined {
  if (options.maxEntries !== undefined && options.maxEntries < 1) {
    return Error(`maxEntries(${options.maxEntries}): must be greater than 0`);
  }

  if (options.replacementPolicy === AgingCacheReplacementPolicy.FIFO && options.ageLimit * 60 <= options.purgeInterval) {
...
export function createAgingCache<TKey, TValue>(
  hierarchy: IStorageHierarchy<TKey, TValue>,
  options?: IAgingCacheOptions): IAgingCache<TKey, TValue> {
  ...
  return new AgingCache<TKey, TValue>(hierarchy, evictQueue, setStrategy, deleteStrategy, options.purgeInterval);
}
