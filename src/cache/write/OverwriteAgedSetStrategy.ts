...
export class OverwriteAgedSetStrategy<TKey, TValue> 
  extends AgingCacheWriteStrategy<TKey, TValue> implements IAgingCacheSetStrategy<TKey, TValue> {
  ...
  set(key: TKey, value: TValue, force: boolean): Promise<AgingCacheWriteStatus> {
    ...
    return this.hierarchy.getValueAtTopLevel(key)
      .then(highestAgedValue => {
        ...
        AgingCacheWriteStrategy.logger.debug(`Set deferred: key=${key},ageToSet=${currentAge},ageFound=${highestAgedValue.age}`)
